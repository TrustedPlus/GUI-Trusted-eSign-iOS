...
#include "../../include/wrapper/pki/attr_vals.h"

AttributeValueCollection::AttributeValueCollection(::TrustedHandle<Attribute> data) {
	LOGGER_FN();
...
::TrustedHandle<std::string> AttributeValueCollection::items(int index) {
	LOGGER_FN();
	...
	::TrustedHandle<std::string> res = new std::string((char *)out, len);
