...
#include "../../include/wrapper/utils/csp.h"
	...
	try {
		...
		if (hCryptProv) {
			if (!CryptReleaseContext(hCryptProv, 0)) {
				THROW_EXCEPTION(0, Csp, NULL, "CryptReleaseContext. Error: 0x%08x", NULL);//GetLastError());
			}
		}
...
bool Csp::checkCPCSPLicense() {
	...
	try {
		...
		if (!CryptAcquireContext(
			...
			CRYPT_VERIFYCONTEXT))
		{
			THROW_EXCEPTION(0, Key, NULL, "CryptAcquireContext. Error: 0x%08x", NULL);//GetLastError());
		}
		...
		if (!CryptGetProvParam(
			...
			0))
		{
			THROW_EXCEPTION(0, Key, NULL, "CryptGetProvParam. Error: 0x%08x", NULL);//GetLastError());
		}
...
::TrustedHandle<std::string> Csp::getCPCSPLicense() {
	//LOGGER_FN();
	...
	try {
		...
		DWORD cbData = 0;
		LPBYTE pbData;
		::TrustedHandle<std::string> license;
		...
		if (!CryptAcquireContext(
			...
			CRYPT_VERIFYCONTEXT))
		{
			THROW_EXCEPTION(0, Csp, NULL, "CryptAcquireContext. Error: 0x%08x", NULL);//GetLastError());
		}
...
::TrustedHandle<std::string> Csp::getCPCSPVersion() {
	//LOGGER_FN();
...
::TrustedHandle<std::string> Csp::getCPCSPVersionPKZI() {
	//LOGGER_FN();
...
::TrustedHandle<std::string> Csp::getCPCSPVersionSKZI() {
	//LOGGER_FN();
...
::TrustedHandle<std::string> Csp::getCPCSPSecurityLvl() {
	//LOGGER_FN();
	...
	try {
		...
		static HCRYPTPROV hCryptProv = 0;
		::TrustedHandle<std::string> version;
		DWORD dwVersion[20];
	...
	try {
		...
		while (CryptEnumProviders(dwIndex, NULL, 0, &dwType, NULL, &cbName))
		{
			...
			if (!CryptEnumProviders(dwIndex++, NULL, NULL, &dwType, pszName, &cbName)) {
				THROW_EXCEPTION(0, Csp, NULL, "CryptEnumProviders. Error: 0x%08x", NULL);//GetLastError());
			}
...
std::vector<::TrustedHandle<std::string>> Csp::enumContainers(int provType, ::TrustedHandle<std::string> provName) {
	...
	try {
#ifdef CSP_ENABLE
		std::vector<::TrustedHandle<std::string>> res;
...
::TrustedHandle<Certificate> Csp::getCertifiacteFromContainer(::TrustedHandle<std::string> contName, int provType, ::TrustedHandle<std::string> provName) {
	//LOGGER_FN();
	...
	try {
		...
		if (!CryptGetUserKey(hProv, AT_SIGNATURE, &hKey)) {
			...
			if (!CryptGetUserKey(hProv, AT_KEYEXCHANGE, &hKey)) {
				THROW_EXCEPTION(0, Csp, NULL, "CryptGetUserKey. Error: 0x%08x", NULL);//GetLastError());
			}
			else {
		...
		if (!CryptGetKeyParam(hKey, KP_CERTIFICATE, NULL, &cbName, 0)) {
			DWORD ee = 0;//GetLastError();
			THROW_EXCEPTION(0, Csp, NULL, "CryptGetKeyParam. Error: 0x%08x", NULL);//GetLastError());
		}
		...
		if ((pCertContext = CertCreateCertificateContext(X509_ASN_ENCODING, pbCertificate, cbName)) == NULL) {
			THROW_EXCEPTION(0, Csp, NULL, "CertCreateCertificateContext. Error: 0x%08x", NULL);//GetLastError());
		}
...
void Csp::installCertifiacteFromContainer(::TrustedHandle<std::string> contName, int provType, ::TrustedHandle<std::string> provName) {
	LOGGER_FN();
	...
	try {
		...
		wide_string_len = 0;//MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, (LPCSTR)contName->c_str(), value_len, (LPWSTR)wide_buf, wide_string_len);
		...
		{
			THROW_EXCEPTION(0, Csp, NULL, "CertSetCertificateContextProperty: Code: %d", NULL);//GetLastError());
		};
		...
		if (HCRYPT_NULL == (hCertStore = CertOpenStore(
			...
			L"MY")))
		{
			THROW_EXCEPTION(0, Csp, NULL, "CertOpenStore failed: Code: %d", NULL);//GetLastError());
		}
		...
		{
			THROW_EXCEPTION(0, Csp, NULL, "CertAddCertificateContextToStore failed. Code: %d", NULL);//GetLastError())
		}
...
void Csp::deleteContainer(::TrustedHandle<std::string> contName, int provType, ::TrustedHandle<std::string> provName) {
	LOGGER_FN();
...
::TrustedHandle<std::string> Csp::getContainerNameByCertificate(::TrustedHandle<Certificate> cert, ::TrustedHandle<std::string> category){
	LOGGER_FN();
	...
	try {
		...
		if (HCRYPT_NULL == (hCertStore = CertOpenStore(
			...
			wCategory.c_str())))
		{
			THROW_EXCEPTION(0, Csp, NULL, "CertOpenStore failed: %s Code: %d", category->c_str(), NULL);//GetLastError());
		}
		...
		if (!CryptImportPublicKeyInfo(
			...
			&hPublicKey))
		{
			THROW_EXCEPTION(0, Csp, NULL, "Error during CryptImportPublicKeyInfo. Error: 0x%08x", NULL);//GetLastError());
		}
		...
		{
			THROW_EXCEPTION(0, Csp, NULL, "CryptGetProvParam. Error: 0x%08x", NULL);//GetLastError());
		}
		...
		if (hPublicKey)
		{
			...
			{
				THROW_EXCEPTION(0, Csp, NULL, "CryptDestroyKey. Error: 0x%08x", NULL);//GetLastError());
			}
		}
...
PCCERT_CONTEXT Csp::createCertificateContext(::TrustedHandle<Certificate> cert) {
	LOGGER_FN();
...
bool Csp::findExistingCertificate(
	...
	try {
		...
		THROW_EXCEPTION(0, Csp, e, "Error find certificate in store. Code: %d", NULL);//GetLastError());
	}
}
...
CRYPT_KEY_PROV_INFO * Csp::getCertificateContextProperty(
	...
	try {
		...
		if (!CertGetCertificateContextProperty(pCertContext, dwPropId, NULL, &dwSize)) {
			THROW_EXCEPTION(0, Csp, NULL, "CertGetCertificateContextProperty(NULL) failed. Code: %d", NULL);//GetLastError());
		}
