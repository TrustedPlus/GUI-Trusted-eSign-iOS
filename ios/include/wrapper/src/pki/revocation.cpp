...
#include "../../include/wrapper/pki/revocation.h"

::TrustedHandle<CRL> Revocation::getCrlLocal(::TrustedHandle<Certificate> cert, ::TrustedHandle<PkiStore> pkiStore){
	LOGGER_FN();

	try{
		int ret;
		::TrustedHandle<Filter> filter = new Filter();
		filter->types.push_back(new std::string("CRL"));

		::TrustedHandle<PkiItemCollection> filteredItems = pkiStore->find(filter);
...
bool Revocation::checkCrlTime(::TrustedHandle<CRL> hcrl) {
	LOGGER_FN();
...
std::vector<std::string> Revocation::getCrlDistPoints(::TrustedHandle<Certificate> cert){
	LOGGER_FN();
