...
#define THROW_EXCEPTION(code, className, stack, msg, ...){\
	...
	::TrustedHandle<Exception> __e = new Exception(false, __FILE__, __LINE__, code, #className, __FUNCTION__, stack, msg, ## __VA_ARGS__);\
	throw __e;}
...
#define THROW_OPENSSL_EXCEPTION(code, className, stack, msg, ...) \
	{ \
		::TrustedHandle<Exception> __e = new Exception(true, __FILE__, __LINE__, code, #className, __FUNCTION__, stack, msg, ## __VA_ARGS__); \
		LOGGER_WARN("%s", __e->what()); \
		throw __e; \
	}

#define CATCH_EXCEPTIONS(actions) \
	catch (::TrustedHandle<Exception>& ex)\
	{ \
...
protected:
	...
	va_list arguments_;
	int code_;
	::TrustedHandle<Exception> except_; //::TrustedHandle ||| sd::list
};
