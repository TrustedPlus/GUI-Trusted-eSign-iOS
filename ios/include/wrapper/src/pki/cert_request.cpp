...
CertificationRequest::CertificationRequest(::TrustedHandle<CertificationRequestInfo> csrinfo) :SSLObject<X509_REQ>(X509_REQ_new(), &so_X509_REQ_free){
	LOGGER_FN();
...
void CertificationRequest::sign(::TrustedHandle<Key> key, const char* digest){
	LOGGER_FN();
...
::TrustedHandle<std::string> CertificationRequest::getPEMString(){
	LOGGER_FN();
...
void CertificationRequest::read(::TrustedHandle<Bio> in, DataFormat::DATA_FORMAT format){
	LOGGER_FN();
