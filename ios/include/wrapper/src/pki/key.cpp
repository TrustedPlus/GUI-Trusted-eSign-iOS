...
void Key::readPrivateKey(::TrustedHandle<Bio> in, DataFormat::DATA_FORMAT format, ::TrustedHandle<std::string> password) {
	try{
		if (in.isEmpty()){
...
void Key::readPublicKey(::TrustedHandle<Bio> in, DataFormat::DATA_FORMAT format) {
	try{
		if (in.isEmpty()){
			THROW_EXCEPTION(0, Key, NULL, "Bio is empty");
		}
...
void Key::writePrivateKey(::TrustedHandle<Bio> out, DataFormat::DATA_FORMAT format, ::TrustedHandle<std::string> password) {
	try{
		if (out.isEmpty()){
			THROW_EXCEPTION(0, Key, NULL, "Bio is empty");
		}
...
void Key::writePublicKey(::TrustedHandle<Bio> out, DataFormat::DATA_FORMAT format) {
	try{
		LOGGER_FN();
...
::TrustedHandle<Key> Key::generate(DataFormat::DATA_FORMAT format, PublicExponent::Public_Exponent pubEx, int keySize) {
	LOGGER_FN();
...
int Key::compare(::TrustedHandle<Key> key) {
	LOGGER_FN();
...
::TrustedHandle<Key> Key::duplicate(){
	::TrustedHandle<Key> dkey = new Key(this->internal());
