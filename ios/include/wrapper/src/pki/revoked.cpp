...
#include "../../include/wrapper/pki/revoked.h"

::TrustedHandle<std::string> Revoked::getRevocationDate()
{
	LOGGER_FN();
...
::TrustedHandle<std::string> Revoked::getSerialNumber() {
	LOGGER_FN();
...
::TrustedHandle<std::string> Revoked::getReason()
{
	LOGGER_FN();
...
::TrustedHandle<Revoked> Revoked::duplicate(){
	LOGGER_FN();
