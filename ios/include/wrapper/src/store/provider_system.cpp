...
#include "../../include/wrapper/store/provider_system.h"

Provider_System::Provider_System(::TrustedHandle<std::string> folder){
	LOGGER_FN();
...
void Provider_System::init(::TrustedHandle<std::string> folder){
	...
	for (int i = 0, c = sizeof(listCertStore) / sizeof(*listCertStore); i < c; i++){
		...
		while ((ent = readdir(dir)) != NULL) {
			...
			if (BIO_read_filename(bioFile, uri.c_str()) > 0){
				::TrustedHandle<PkiItem> itemTemp = new PkiItem();
	...
	try{
		...
		if ((dir = FindFirstFile(folder->c_str(), &file_data)) == INVALID_::TrustedHandle_VALUE){
		...
		for (int i = 0, c = sizeof(listCertStore) / sizeof(*listCertStore); i < c; i++){
			...
			if ((dir = FindFirstFile(tempdirInCertStore.c_str(), &file_data)) == INVALID_::TrustedHandle_VALUE){			
			...
			do {
				...
				LOGGER_OPENSSL(BIO_read_filename);
				if (BIO_read_filename(bioFile, uri.c_str()) > 0){
					::TrustedHandle<PkiItem> itemTemp = objectToPKIItem(new std::string(uri));
					if (!itemTemp.isEmpty()){
						providerItemCollection->push(itemTemp);
					}					
				}
...
::TrustedHandle<PkiItem> Provider_System::objectToPKIItem(::TrustedHandle<std::string> uri){
	LOGGER_FN();

	::TrustedHandle<PkiItem> item;

	::TrustedHandle<Bio> in =  new Bio(BIO_TYPE_FILE, uri->c_str(), "rb");
	...
	try{
		...
		::TrustedHandle<Certificate> hcert = NULL;
		::TrustedHandle<CRL> hcrl = NULL;
		...
		if (itPrivateKey(uri, &enc)){
			...
			::TrustedHandle<std::string> keyHash = new std::string(uri->c_str());
		...
		if (!hcert.isEmpty()){
			...
			char * hexHash;
			::TrustedHandle<std::string> hhash = hcert->getThumbprint();
		...
		if (xreq){
			...
			::TrustedHandle<std::string> hhash = new std::string((char *)hash, hashlen);
			...
			::TrustedHandle<std::string> nameRes = new std::string(str_name.c_str(), str_name.length());
			item->csrSubjectName = nameRes;

			/* Request subject friendly name */
			::TrustedHandle<std::string> friendlyName = new std::string("");
		...
		if (!hcrl.isEmpty()){
			...
			char * hexHash;
			::TrustedHandle<std::string> hhash = hcrl->getThumbprint();
			PkiStore::bin_to_strhex((unsigned char *)hhash->c_str(), hhash->length(), &hexHash);
			item->hash = new std::string(hexHash);
...
::TrustedHandle<Certificate> Provider_System::getCertFromURI(::TrustedHandle<std::string> uri, ::TrustedHandle<std::string> format){
	LOGGER_FN();
...
::TrustedHandle<CRL> Provider_System::getCRLFromURI(::TrustedHandle<std::string> uri, ::TrustedHandle<std::string> format){
	LOGGER_FN();
...
::TrustedHandle<CertificationRequest> Provider_System::getCSRFromURI(::TrustedHandle<std::string> uri, ::TrustedHandle<std::string> format){
	LOGGER_FN();
...
::TrustedHandle<Key> Provider_System::getKeyFromURI(::TrustedHandle<std::string> uri, ::TrustedHandle<std::string> format, bool enc){
	LOGGER_FN();
...
void Provider_System::addPkiObject(::TrustedHandle<std::string> uri, ::TrustedHandle<Certificate> cert){
	LOGGER_FN();

	try{
		::TrustedHandle<Bio> out = new Bio(BIO_TYPE_FILE, uri->c_str(), "wb");
...
void Provider_System::addPkiObject(::TrustedHandle<std::string> uri, ::TrustedHandle<CRL> crl){
	LOGGER_FN();
...
void Provider_System::addPkiObject(::TrustedHandle<std::string> huri, ::TrustedHandle<Key> key, ::TrustedHandle<std::string> password){
	LOGGER_FN();

	try{
		::TrustedHandle<Bio> out = new Bio(BIO_TYPE_FILE, huri->c_str(), "wb");
...
void Provider_System::addPkiObject(::TrustedHandle<std::string> uri, ::TrustedHandle<CertificationRequest> csr){
	LOGGER_FN();
...
::TrustedHandle<std::string> Provider_System::getKey(::TrustedHandle<std::string> objectPatch){
	LOGGER_FN();

	try{
		BIO *bioKeyFile = NULL;
		::TrustedHandle<std::string> key_file_name;
		size_t lastindex;

		::TrustedHandle<std::string> key_hash = new std::string(objectPatch->c_str());
...
bool Provider_System::itPrivateKey(::TrustedHandle<std::string> uri, int *enc){
	LOGGER_FN();
