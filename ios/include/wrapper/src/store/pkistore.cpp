...
#include "../../include/wrapper/store/pkistore.h"
...
#endif
#if defined(CPROCSP)
	#include "../../include/wrapper/store/provider_cryptopro.h"
#endif

PkiStore::PkiStore(::TrustedHandle<std::string> json){
	LOGGER_FN();
	...
	try{
		...
		THROW_EXCEPTION(0, PkiStore, e, "Cannot be constructed PkiStore(::TrustedHandle<std::string> json)");
	}	
}

::TrustedHandle<PkiItemCollection> PkiStore::find(::TrustedHandle<Filter> filter){
	LOGGER_FN();
...
::TrustedHandle<PkiItem> PkiStore::findKey(::TrustedHandle<Filter> filter){
	LOGGER_FN();
	...
	try{
		...
		::TrustedHandle<PkiItem> key = new PkiItem();
		...
		for (int i = 0, c = storeItemCollection->length(); i < c; i++){
			...
			if (result){
				::TrustedHandle<std::string> keyHash;
...
::TrustedHandle<Certificate> PkiStore::getItemCert(::TrustedHandle<PkiItem> item){
	LOGGER_FN();
	...
	try{
		...
		::TrustedHandle<Certificate> cert = NULL;
...
::TrustedHandle<CRL> PkiStore::getItemCrl(::TrustedHandle<PkiItem> item){
	LOGGER_FN();

	try{
		::TrustedHandle<CRL> crl = new CRL();
...
::TrustedHandle<CertificationRequest> PkiStore::getItemReq(::TrustedHandle<PkiItem> item){
	LOGGER_FN();

	try{
		::TrustedHandle<CertificationRequest> csr = new CertificationRequest();
...
::TrustedHandle<Key> PkiStore::getItemKey(::TrustedHandle<PkiItem> item){
	LOGGER_FN();
...
::TrustedHandle<PkiItemCollection> PkiStore::getItems(){
	LOGGER_FN();

	return this->storeItemCollection;	
}

::TrustedHandle<CertificateCollection> PkiStore::getCerts(){
	LOGGER_FN();

	try{
		::TrustedHandle<CertificateCollection> result = new CertificateCollection();
		...
		for (int i = 0; i < providers->length(); i++){
			::TrustedHandle<PkiItemCollection> tempColl;
			...
			for (int j = 0; j < tempColl->length(); j++){
				if (strcmp(tempColl->items(j)->type->c_str(), "CERTIFICATE") == 0){
					::TrustedHandle<Certificate> cert = getItemCert(tempColl->items(j));
					result->push(cert);
				}
			}
		}
...
void PkiStore::addProvider(::TrustedHandle<Provider> provider) {
	LOGGER_FN();
	
	providers->push(provider);

	::TrustedHandle<PkiItemCollection> tempColl = provider->getProviderItemCollection();
	for (int i = 0; i < tempColl->length(); i++) {
		this->storeItemCollection->push(tempColl->items(i));
	}
}

::TrustedHandle<std::string> PkiStore::addPkiObject(::TrustedHandle<Provider> provider, ::TrustedHandle<std::string> category, ::TrustedHandle<Certificate> cert){
	LOGGER_FN();
	...
	try{
		if (strcmp(provider->type->c_str(), "SYSTEM") == 0){
			...
			::TrustedHandle<std::string> huri = new std::string(uri);
...
::TrustedHandle<std::string> PkiStore::addPkiObject(::TrustedHandle<Provider> provider, ::TrustedHandle<std::string> category, ::TrustedHandle<CRL> crl){
	LOGGER_FN();
...
::TrustedHandle<std::string> PkiStore::addPkiObject(::TrustedHandle<Provider> provider, ::TrustedHandle<std::string> category, ::TrustedHandle<CertificationRequest> csr){
	LOGGER_FN();
...
::TrustedHandle<std::string> PkiStore::addPkiObject(::TrustedHandle<Provider> provider, ::TrustedHandle<Key> key, ::TrustedHandle<std::string> password){
	LOGGER_FN();
	...
	try{
		...
		if (strcmp(provider->type->c_str(), "SYSTEM") == 0) {
			...
			if (pkey->type == EVP_PKEY_RSA)
				...
				::TrustedHandle<std::string> res = new std::string(reinterpret_cast<char*>(hexHash));
...
void PkiStore::deletePkiObject(::TrustedHandle<Provider> provider, ::TrustedHandle<std::string> category, ::TrustedHandle<Certificate> cert){
	LOGGER_FN();
