...
class TrustedHandle {
public:
	TrustedHandle(T* realPtr = 0);
	TrustedHandle(const TrustedHandle& rhs);
	~TrustedHandle();
	TrustedHandle& operator=(const TrustedHandle& rhs);
	...
	TrustedHandle& empty() // release()
	{
		*this = TrustedHandle(NULL);
		return *this;
	}
...
template<class T>
TrustedHandle<T>::TrustedHandle(T* realPtr)
: counter(new CountHolder) {
...
template<class T>
TrustedHandle<T>::TrustedHandle(const TrustedHandle& rhs)
: counter(rhs.counter) {
	init();
}

template<class T>
TrustedHandle<T>::~TrustedHandle() {
	counter->removeReference();
}

template<class T>
TrustedHandle<T>& TrustedHandle<T>::operator=(const TrustedHandle& rhs) {
...
template<class T>
T* TrustedHandle<T>::operator->() const {
	if (!counter->pointee)
...
template<class T>
T* TrustedHandle<T>::attach(T* realPtr) {
	T* oldValue = counter->pointee;
	...
	if (counter->pointee != realPtr) {
		...
		*this = *new TrustedHandle(realPtr);
	}
