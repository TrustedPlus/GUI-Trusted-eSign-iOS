...
import { addCert, addCertForEnc } from "../actions/index";
						...
						{(certificates.filter((cert) => ((cert.category.toUpperCase() === "OTHERS") || (cert.category.toUpperCase() === "ADDRESSBOOK")))).length !== 0 ?
							<Button style={{ width: "100%", backgroundColor: "white" }} onPressIn={() => this.setState({ collapsedOtherCert: !this.state.collapsedOtherCert })}>
								...
								{this.state.collapsedOtherCert
									? <Icon style={{ color: "grey", position: "absolute", right: "5%" }} name="ios-arrow-down" />
									: <Icon style={{ color: "grey", position: "absolute", right: "5%" }} name="ios-arrow-up" />}
							</Button> : (certificates.filter((cert) => (cert.category.toUpperCase() === "MY") && (cert.hasPrivateKey))).length !== 0 ? null : <Text style={[styles.sign_enc_prompt, { paddingTop: "50%" }]}>Сертификатов нет. Нажмите кнопку 'добавить' для импорта или создания сертификата</Text>}
