...
#include "../../include/wrapper/cms/cmsRecipientInfos.h"

void CmsRecipientInfoCollection::push(::TrustedHandle<CmsRecipientInfo> ri) {
	LOGGER_FN();
...
::TrustedHandle<CmsRecipientInfo> CmsRecipientInfoCollection::items(int index) {
	LOGGER_FN();
	...
	::TrustedHandle<CmsRecipientInfo> cmsRecInfo = new CmsRecipientInfo();
	cmsRecInfo->ri = ri;
