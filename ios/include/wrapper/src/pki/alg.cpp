#include "../stdafx.h"
#include "../../include/wrapper/pki/alg.h"
...
Algorithm::Algorithm(::TrustedHandle<OID> alg_oid)
	:SSLObject<X509_ALGOR>(X509_ALGOR_new(), &so_X509_ALGOR_free){
	LOGGER_FN();
	...
	try{
		::TrustedHandle<std::string> sn = alg_oid->getShortName();
		init(sn->c_str());
	}
	catch (::TrustedHandle<Exception> e){
		THROW_EXCEPTION(0, Algorithm, e, "Can not init Algorithm");
...
::TrustedHandle<Algorithm> Algorithm::duplicate() {
	LOGGER_FN();
...
::TrustedHandle<OID> Algorithm::getTypeId() {
	LOGGER_FN();

	return new OID(this->internal()->algorithm, this->TrustedHandle());
}

::TrustedHandle<std::string> Algorithm::getName() {
	LOGGER_FN();
