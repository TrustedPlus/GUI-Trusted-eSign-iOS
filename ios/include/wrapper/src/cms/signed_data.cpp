...
#include "../../include/wrapper/cms/signed_data.h"

::TrustedHandle<CertificateCollection> SignedData::certificates(){
	LOGGER_FN();
...
::TrustedHandle<Certificate> SignedData::certificates(int index){
	LOGGER_FN();

	return this->certificates()->items(index);
}

::TrustedHandle<SignerCollection> SignedData::signers(){
	LOGGER_FN();

	stack_st_CMS_SignerInfo *signers_ = CMS_get0_SignerInfos(this->internal());

	return new SignerCollection(signers_, this->TrustedHandle());
}

::TrustedHandle<Signer> SignedData::signers(int index){
	LOGGER_FN();
...
void SignedData::read(::TrustedHandle<Bio> in, DataFormat::DATA_FORMAT format){
	LOGGER_FN();
	...
	try{
		...
		case DataFormat::DER:
			...
			if (ci && !CMS_is_detached(ci)){
				...
				::TrustedHandle<Bio> content = new Bio(BIO_new_mem_buf(asn->data, asn->length));
...
void SignedData::write(::TrustedHandle<Bio> out, DataFormat::DATA_FORMAT format){
	LOGGER_FN();
...
::TrustedHandle<Signer> SignedData::createSigner(::TrustedHandle<Certificate> cert, ::TrustedHandle<Key> pkey){
	LOGGER_FN();
	...
	return new Signer(signer, this->TrustedHandle());
}

void SignedData::addCertificate(::TrustedHandle<Certificate> cert){
	LOGGER_FN();

	::TrustedHandle<Certificate> cert_copy = cert->duplicate();
	...
	cert_copy->setParent(this->TrustedHandle());
}

void SignedData::setContent(::TrustedHandle<Bio> value){
	LOGGER_FN();

	this->content = value;
}

::TrustedHandle<Bio> SignedData::getContent(){
	LOGGER_FN();
...
bool SignedData::verify(::TrustedHandle<CertificateCollection> certs){
	LOGGER_FN();
	int res;
...
::TrustedHandle<SignedData> SignedData::sign(::TrustedHandle<Certificate> cert, ::TrustedHandle<Key> pkey, ::TrustedHandle<CertificateCollection> certs, ::TrustedHandle<Bio> content, unsigned int flags){
	LOGGER_FN();
	...
	::TrustedHandle<SignedData> sd = new SignedData(res);
	sd->flags = flags;
