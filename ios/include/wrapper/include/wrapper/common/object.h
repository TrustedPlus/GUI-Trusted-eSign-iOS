...
class SObject{
public:
	SObject(void *obj, ::TrustedHandle<SObject>parent, const char* tname)
...
class SSLObject{
public:
	SSLObject(T* data, void(*fn)(void *), ::TrustedHandle<SObject> parent = NULL) :fnFree_(fn){
	...
	T *internal()
	{
		...
		::TrustedHandle<SObject> tmp = this->data_;
		return tmp->internal<T>();
	}

	::TrustedHandle<SObject> TrustedHandle(){
		LOGGER_FN();

		return this->data_;
	}

	void setParent(::TrustedHandle<SObject >v){
		LOGGER_FN();
...
#define SSLOBJECT_new(class_name, type) \
	class_name(type *data, ::TrustedHandle<SObject> parent = NULL) \
