...
import { readCertKeys, deleteCertAction } from "../actions/CertKeysAction";
...
interface PropertiesCertProps {
   ...
   cert: string;
   readCertKeys(): void;
   deleteCertAction(CN: string): void;
}
...
export class PropertiesCert extends React.Component<PropertiesCertProps> {
   ...
   deleteCert() {
      AlertIOS.alert(
         "Вы уверены, что хотите удалить сертификат '" + this.props.navigation.state.params.cert.subjectFriendlyName + "'?",
         null,
         [
            {
               text: "Ок", onPress: () => {
                  let subjectFriendlyName = this.props.navigation.state.params.cert.subjectFriendlyName;
                  NativeModules.Wrap_Cert.deleteCertInStore(
                     ...
                     (err, deleteCert) => {
                        if (err) {
                           ...
                           this.props.deleteCertAction(subjectFriendlyName);
